...
Omega = 2*pi*116;                       % Angular velocity of rotor [rad/s]
rho = 0.05;                              % Blade lenght [m]
...
RCS = 0.01;                             % RCS of drone [m^2]
drone_l = 202e-3;                        % Drone mean lenght [m]
...
% 5. Compute dr,dt e tdwell. (dr is already given as input parameter,
% dt is the chirp duration)


% Compute dwell time in order to satisfy N_mig_peak

t_dwell_min = 0.1;%N_mig_peak*T_migr;       % Minimum dwell time [s]
...
T_chirp = 1.3e-3;%T_chirp_max;             % Chirp duration [s]
...
delta = ((-drone_l/2) + (drone_l)*rand(2,1));               % Position of each rotor in relation to CM (Only elevation angle is considered)
delta_final = [delta(1);-delta(1);delta(2);-delta(2)];      % Variation of each rotor from CM
...
for r=1:N_r
    ...
    for k=0:N_b-1
        ...
        phi = atan((lambda*mu)/(c*Omega)) -pi/2 + phi_0(r);

        % Range variation formula
        r_var(:,k+1) = ( R0 + ((rho/(lambda*mu))*sqrt(((c^2)*(Omega^2))+((lambda^2)*(mu^2))).*cos(Omega.*ts + phi + k*2*pi/(N_b)) ));
    
        plot(ts,r_var(:,k+1))
        yline(R0)    % Plot fixed rotor contribution at R0+delta distance from radar
...
P_tx = p_tx_req(R_max, SNR_min, A_eq, RCS, T_chirp, T_s, lambda, L_linear);
...
function [P_tx] = p_tx_req(R_max, SNR, A_eq, RCS, T_chirp, T_s, lambda,L)

k_b = physconst('Boltzmann');

P_tx =   ( SNR * (4*pi) * (lambda^2) * ((R_max)^4) * k_b * T_s*L) / ( (A_eq^2) * RCS * T_chirp );

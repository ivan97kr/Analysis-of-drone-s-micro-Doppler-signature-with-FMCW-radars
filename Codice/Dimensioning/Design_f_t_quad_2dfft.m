...
n_t = 1;                                % Number of time-pixel desidered between two BF
n_f = 1;                                % Number of freq-pizel desidered between two BF
...
function [P_tx] = p_tx_req(R_max, SNR, G, RCS, T_chirp, T_s, lambda, noise_fig)

k_b = physconst('Boltzmann');

P_tx =   ( SNR * (4*pi)^3 * ((R_max)^4) * k_b * T_s * noise_fig) / ( (G^2) * lambda^2 * RCS * T_chirp );

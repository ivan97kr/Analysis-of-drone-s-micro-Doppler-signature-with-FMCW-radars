...
noise_figure_db = 10;                       % Noise figure in dB
noise_figure_linear = 10^(noise_figure_db/10);                       % Noise figure in linear
...
Omega = 2*pi*100;                        % Angular velocity of rotor [rad/s]
rho = 0.1;                              % Blade lenght [m]
...
N_fft_max = 2*dt_req/T_chirp_min_range;
...
for N_fft=N_fft_min:2:N_fft_max
    ...
    T_chirp_max_res = (T_BF*2)/(n_t*N_fft);
    ...
    while i == 0
        ...
        N_fft_max_opt = 2*dt_req/T_chirp_min_range;
        ...
        if any(T_chirp_min_res > T_chirp_max_res | T_chirp_max_res < T_chirp_min_range) %|  N_fft< N_fft_min_opt | N_fft>N_fft_max_opt | T_chirp_max_res < T_chirp_min_range | T_chirp_min_res>T_chirp_max_radar)
    ...
    T_chirp_max_res = (T_BF*2)/(n_t*N_fft);          % [s]   
    T_chirp_min_res = n_f/(N_fft*f_max);


    % Choose T_chirp max value compatible with T_chirp radar constraint
    while (T_chirp_max_res>T_chirp_max_radar)
        T_chirp_max_res = T_chirp_max_radar;  % set to maximum possible
    end

    while (T_chirp_min_res<T_chirp_min_range)
        T_chirp_min_res = T_chirp_min_range;
    end



    % Check if T_chirp max staisfy the unambiguous radar range condition

    if any(T_chirp_max_res < T_chirp_min_range)
        disp('Max T_chirp does not satisfy unambiguous range condition, you can try to relax K_r (unanmbiguous range cost.)') 
        return;
    end
    ...
    dt_final = N_fft*T_chirp/2;           % Time resolution with chirp duration chosen
    ...
    N_fft_max_final = 2*dt_final/T_chirp_min_range;
    ...
    P_tx = p_tx_req(R_max, SNR_fft, G, rcs_drone, T_chirp, T_s, lambda, noise_figure_linear);
    text = ['P_tx = ', num2str(P_tx),' W'];
    disp(text)
    P_ap = P_tx*A_eq;
    text = ['P_ap = ', num2str(P_ap),' W*m^2'];
    disp(text)
...
for r=1:N_r
    for k=0:N_b-1
        phi = Omega*ts + phi_0(r) + k*2*pi/N_b;
...
function [P_tx] = p_tx_req(R_max, SNR, G, RCS, T_chirp, T_s, lambda, noise_figure)

k_b = physconst('Boltzmann');

P_tx =   ( SNR * (4*pi)^3 * ((R_max)^4) * k_b * T_s * noise_figure) / ( (G^2) * lambda^2 * RCS * T_chirp );

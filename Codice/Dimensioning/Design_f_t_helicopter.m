...
t_dwell_min = 0.08;       % Minimum dwell time [s]
...
for N_fft=N_fft_min:2:N_fft_max
    ...
    while (T_chirp_max_res>T_chirp_max_radar)
        T_chirp_max_res = T_chirp_max_res-0.001e-6;  % decrease by 1 microsec at time
    end
    ...
    T_chirp = 66.7e-6;             % Chirp duration [s]
    ...
    disp(text)
    text = ['Power per Aperture= ', num2str(P_ap),' W*m^2'];
    disp(text)
